...
// https://stackoverflow.com/questions/19471235/sum-of-two-chars-in-c-c  (character addition)
// https://www.tutorialspoint.com/character-arithmetic-in-c(practical)
// char is of one byte that means it store maximum of 2^8=256 that means 256 unsigned numbers ke corresponding character exists otherwise overflow(uss nos.ke coresponding koi character nhi)
// take care of signed and unsigned char ranges
// https://stackoverflow.com/questions/13161199/ascii-table-negative-value#:~:text=There%20is%20no%20such%20thing,don't%20use%20plain%20ASCII. // negative ascii
...
// sb calculation ascii mai kheli hai
...
{ /* string passed by user from main file woh yaha argument mai ayega */
    ...
    for (unsigned int i = 0; i < nofdigits; i++)
    {
        ...
        if (c >= '0' && c <= '9')
        {
            ret.digits[i] = c - '0'; /* digits ki jo char array bni hai jisme hum aak slot mai aak aak digit(in numerical form but in char aray(eg ('9' -'0') is '\t' for 9 is store in memory(ascii no. of '\t'is 9)),digit[0] is '\t' ,int(digit[0]) is 9(char add means unka int addition hi hota(ascii)),in addition operation (digit[0] +digit[1])ascii value direct add,(hamara aim ki koi bhi digit ko from string extraction (char) ko less space mai store krle and usko hum use bhi krle for operation(integer jaisa rkhle in char array) is fullfilled) for less space as compare to int array(becoz int 4 byte and char 1 byte) but aisa like no. ko char array mai store krke usko use lelo operations mai aisa bss digits ke liye kr skte becoz char c='9'ye character hai char c='10'character ni hai it is string and max value of char in int is 256(addition of character may lead to overflow (exceed 255)but not the case when we add two characters digits becoz sum is <255) rkhenge extraction from string in such a way string ka last yaha ka first for easier operation becoz we are doing operations from left to right which is easier to look at on a computer */
        }                            // digits mai already like c ka decimal form store hai in characters.
        else
...
// two character addition means unka ascii addition and then uss ascii ke corresponding character we get if we store that ascii in a char variable.
